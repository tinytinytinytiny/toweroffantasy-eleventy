	...
	render({ name, string: { element } }) {
		return {
			...
			awakening: [
				...
				`Maximum Frost points increased to **15**. Using a ${element.ice.name} weapon’s Discharge Skill inside the ice field inflicts (Frost points + 5) × **25%** × ${element.ice.name} ATK to all enemies in the field.`,
				'Increase the current weapon’s base ATK growth by **32%**.',
				`Using a ${element.ice.name} weapon to break a shield inside the ice field will freeze the target for **2** seconds and deal an additional (Frost points × **35%** × ${element.ice.name} ATK) damage (doubled if the target cannot be frozen).`,
