module.exports = {
	...
	render({ values, footnote }) {
		return this.range(4).map(i => [
			...
			`Increase Whirlwind’s damage by **${values.set4[0][i]}** and restore HP equal to **${values.set4[1][i]}** of the damage dealt while Whirlwind is active. The maximum HP restored every **0.5** seconds cannot exceed **${values.set4[2][i]}** of ATK.\n\n${this.china()} Converts **${values.set4[3][i]}* of damage dealth during shield to health, restores no more than **${values.set4[4][i]}** of attack power every 0.5 seconds.`
		]);
	}
};
